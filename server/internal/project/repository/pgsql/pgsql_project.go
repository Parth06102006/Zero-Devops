...
func (m *pgSQLProjectRepository) Store(ctx context.Context, p *domain.Project) error {
	...
	query := `
		INSERT INTO projects (
			...
			configured_branch, project_webhook_enabled, desired_revision_generation,
	...
	if err := m.Conn.QueryRowContext(ctx, query,
		...
		p.ConfiguredBranch, p.ProjectWebhookEnabled, p.DesiredRevisionGeneration,
...
//
//	configured_branch, project_webhook_enabled, build_configuration, configuration_version,
//	command_policy_version, command_scan_result, updated_at
//
...
func (m *pgSQLProjectRepository) Update(ctx context.Context, p *domain.Project) error {
	...
	query := `
		UPDATE projects SET
			...
			command_policy_version = $5,
			command_scan_result = $6,
			updated_at = NOW()
		WHERE id = $7 AND user_id = $8
	`
	res, err := m.Conn.ExecContext(ctx, query,
		p.ConfiguredBranch, p.ProjectWebhookEnabled, buildConfig, p.ConfigurationVersion,
		p.CommandPolicyVersion, scanResult, p.ID, p.UserID,
	)
	if err != nil {
		log := appmiddleware.LoggerFromContext(ctx)
